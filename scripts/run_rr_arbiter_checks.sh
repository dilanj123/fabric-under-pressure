...
BUILD="$OUT/sim_work"
mkdir -p "$OUT"
rm -rf "$BUILD" "$OUT/formal_prove" "$OUT/formal_cover" "$OUT/formal_fairness"
...
(
  ...
  sby -f -d "$OUT/formal_fairness" rr_arbiter_fairness.sby fairness
...
test -f "$OUT/formal_fairness/PASS"
