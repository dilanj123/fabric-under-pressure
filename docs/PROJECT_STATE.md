...
**Architecture:** planning frozen; raw-address decode, request-shape legality, manager/internal ID mapping, per-manager/per-direction outstanding tracking, Architecture-A 3-way RR arbiter, per-manager write-owner context and one target-specific Architecture-A AW eligibility/RR boundary exist, project AXI fabric is not integrated.
...
- The target-specific Architecture-A AW boundary composes the frozen eligibility equation with the verified RR primitive, blocks a write-owned target, preserves held grants under READY stall, suppresses D031 same-edge successor lookahead and has a two-target simulation check; no AW payload/READY/VALID integration or production state-allocation fanout exists.

## Next smallest task
Create the next narrow Architecture A task from the frozen dependency structure: integrate the manager-facing and target-facing AW payload/VALID/READY boundaries with the target scheduler while keeping W routing and response routing separate. Do not implement Architecture B, CPU integration or CDC in that task.
