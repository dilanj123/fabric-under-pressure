...
| R-008 write ownership | `tb/directed/axi_write_owner_tb.sv` covers allocation, metadata capture, W-before-owner diagnostics, single/multi/16-beat accounting, stalls, WLAST errors, pre-state recycling and reset; `tb/directed/axi_write_owner_3_tb.sv` covers independent M0/M1/M2 contexts | `formal/axi_write_owner_formal.sv` checks registered context stability, beat accounting, bounds, WLAST release, malformed-event diagnostics, no underflow and next-cycle reallocation; bounded BMC/cover assumptions are recorded in the formal evidence | `rtl/axi_write_owner.sv`; target-specific W routing, AW/W integration, target uniqueness and response paths remain future work |
