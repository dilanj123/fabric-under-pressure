...
| R-003/R-007 IDs and limits | `tb/directed/id_mapper_tb.sv` exhaustively checks 48 valid widen/strip mappings, all 16 reserved return prefixes and collision prevention | `formal/id_mapper_formal.sv` proves exact concatenation, valid-manager round trip, lower-ID preservation, prefix uniqueness and reserved-code validity; busy-ID state/count limits remain future work | `rtl/axi_id_mapper.sv`; outstanding tables and response routing remain future integration |
